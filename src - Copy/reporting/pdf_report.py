...
def _kpi_box(kpis: list[list[str]], *, styles: Any) -> Table:
    ...
    formatted: list[list[Any]] = []
    for k, v in kpis:
        formatted.append([Paragraph(_escape_html(k), label_style), Paragraph(_escape_html(v), value_style)])
...
def _kpi_cover_flowables(*, project_name: str, artifacts_dir: Path, styles: Any) -> list[Any]:
    ...
    prot_changed = len((prot_diff.get("changed") or {}).keys()) if isinstance(prot_diff.get("changed"), dict) else 0
    prot_added = len(prot_diff.get("added") or []) if isinstance(prot_diff.get("added"), list) else 0
    prot_removed = len(prot_diff.get("removed") or []) if isinstance(prot_diff.get("removed"), list) else 0
    ...
    kpis = [
        ...
        ["Nominal tables", f"cables={_fmt_int(cables_n)} protections={_fmt_int(prot_n)} methods={_fmt_int(meth_n)}"],
        ["Overlays", f"{len(overlays) if isinstance(overlays, list) else 0} | prot +{prot_added} -{prot_removed} ~{prot_changed}"],
    ]
...
def build_engineering_pdf(
    *,
    ...
    story: list[Any] = []
    story.append(KeepTogether(_kpi_cover_flowables(project_name=project_name, artifacts_dir=artifacts_dir, styles=styles)))
