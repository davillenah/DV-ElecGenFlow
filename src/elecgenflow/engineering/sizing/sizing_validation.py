# src/elecgenflow/engineering/sizing/sizing_validation.py
...
class SizingRow:
    ...
    voltage_ll_v: float
    ib_a: float
    ib_design_a: float  # ✅ agregado (Ib con reserva)
...
class CableSizingValidationService:
    ...
    def validate_feedrs(
        *,
        ...
        cable_reserve_pct: float = 0.0,  # ✅ agregado
    ) -> dict[str, Any]:
        """
        - Valida Ib vs Iz con Iz desde:
          1) nominal tables (cable tag exacto)
          2) ampacity AEA (si hay catálogo y se puede mapear)
        - Si FAIL/UNKNOWN y hay ampacity, sugiere sección mínima que cumpla (auto-sizing suggested).
        - Usa Ib_design = Ib*(1 + reserve_pct/100)
        """
        ...
        for f in feeders:
            ...
            status = "UNKNOWN"
            if iz is not None:
                status = "PASS" if ib_design <= iz else "FAIL"
            else:
                if not note:
            ...
            if ampacity is not None and aea_key is not None and status in {"FAIL", "UNKNOWN"}:
                sugg_sec = ampacity.suggest_section(key=aea_key, ib_a=ib_design)
        ...
        return {
            ...
            "assumptions": {
                ...
                "ib_design": "Ib_design = Ib*(1 + reserve_pct/100)",
                "iz_sources": [
    ...
    def to_markdown(sizing_report: dict[str, Any]) -> str:
        ...
        lines.append("")
        lines.append("## Feeder Validation (Ib_design vs Iz) + Suggested Section")
        lines.append("")
        lines.append(
            "| From | To | Wire | kVA | Vll(V) | Ib(A) | Ib*(1+R) | Sec(mm²) | Iz(A) | Iz src | Suggested(mm²) | Sug Iz(A) | Status | Note |"
        )
        lines.append("|---|---|---|---:|---:|---:|---:|---:|---:|---|---:|---:|---|---|")
        for r in rows:
            iz = r.get("iz_a")
            ...
            lines.append(
                ...
                f"{r.get('downstream_kva', 0):.3f} | {r.get('voltage_ll_v', 0)} | {r.get('ib_a', 0):.3f} | {r.get('ib_design_a', 0):.3f} | "
