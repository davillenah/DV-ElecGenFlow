...
def _parallel_grouped_from_config(wire_cfg: dict[str, Any]) -> tuple[int, int]:
    meta = wire_cfg.get("meta") or {}
    p = 1
    g = 1
    if isinstance(meta, dict):
        try:
            p = int(meta.get("parallel") or 1)
        except Exception:
            p = 1
        try:
            g = int(meta.get("grouped") or 1)
        except Exception:
            g = 1
    if p < 1:
        p = 1
...
def _wire_config_from_links(links: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
    """
    Construye wire_id -> wire_config desde una lista de links.
    Acepta tanto runtime network_links como compiled_links.
    """
    out: dict[str, dict[str, Any]] = {}
    for lk in links:
        wire_id = str(lk.get("wire_id") or lk.get("wire") or "").strip()
        wc = lk.get("wire_config") or {}
        if wire_id and isinstance(wc, dict) and wc:
            out[wire_id] = wc
    return out
...
class CableScheduleRow:
    ...
    # ✅ nuevos para auditoría
    parallel: int
    grouped: int
    ib_per_cable_a: float
...
class CableScheduleService:
    ...
    def build(
        *,
        ...
        network_links: list[dict[str, Any]] | None = None,
    ) -> dict[str, Any]:
        wc_map: dict[str, dict[str, Any]] = {}
        if network_links:
            wc_map = _wire_config_from_links(network_links)
        if not wc_map:
        ...
        for f in feeders:
            ...
            parallel, grouped = _parallel_grouped_from_config(wire_cfg)

            # ✅ corriente por cable (si hay ternas en paralelo)
            ib_per_cable = ib_design / float(parallel) if parallel >= 1 else ib_design

            key = _key_from_wire_config(wire_cfg) or default_key

            selected_section = ampacity.suggest_section(key=key, ib_a=ib_per_cable)
            selected_iz = (
        ...
        return {
            "version": "0.2",
            "assumptions": {
                "selection": "minimum section that satisfies Iz >= Ib_per_cable",
                "ib_formula": "Ib = kVA*1000 / (sqrt(3)*Vll)",
                "ib_design": "Ib_design = Ib * (1 + cable_reserve_pct/100)",
                "ib_per_cable": "Ib_per_cable = Ib_design / parallel",
            ...
            "rows": [
                {
                    ...
                    "parallel": r.parallel,
                    "grouped": r.grouped,
                    "ib_per_cable_a": round(r.ib_per_cable_a, 3),
    ...
    def to_markdown(schedule: dict[str, Any]) -> str:
        ...
        lines.append(f"- ib_per_cable: {a.get('ib_per_cable')}")
        ...
        lines.append("")
        lines.append(
            "| From | To | WireID | kVA | Vll(V) | Ib(A) | Ib*(1+R) | parallel | grouped | Ib/cable(A) | Cond | Selected (mm²) | Iz(A) | Note |"
        )
        lines.append("|---|---|---|---:|---:|---:|---:|---:|---:|---:|---|---:|---:|---|")

        for r in rows:
            cond = (
                f"{r.get('conductor', '?')}/{r.get('insulation', '?')}/"
                f"{r.get('method', '?')}/{r.get('arrangement', '?')}"
            )
            lines.append(
                f"| {r.get('from', '')} | {r.get('to', '')} | {r.get('wire_id', '')} | "
                f"{r.get('kva', 0):.3f} | {r.get('vll', 0)} | {r.get('ib_a', 0):.3f} | {r.get('ib_design_a', 0):.3f} | "
                f"{r.get('parallel', 1)} | {r.get('grouped', 1)} | {r.get('ib_per_cable_a', 0):.3f} | "
